...
class RandomSpeckleNoise(TransformBase):
    """Add speckle noise on image, with a given probability and an impact factors.
        if random_factors is true, factors will be a random number between 0 and factors_max, else it will be fixed at factors_max.
        if channels=1, the noise will be grayscale, if channels=3, it will be rgb."""
    def __init__(self, probability=0.2, factors_max=0.5, random_factors=True, channels=1):
    ...
    def roll(self):
        add_noise = random.random() < self.probability
        factors = self.factors_max
        if self.random_factors:
            factors *= random.random()
        return add_noise, factors

    def transform_image(self, image, add_noise, factors):
        assert isinstance(image, np.ndarray) , 'Only supported for numpy input'
        if add_noise:
            gauss = np.random.randn(*image.shape[:-1], self.channels)   # (h, w, c)
            image = image + image * gauss * factors
            image = np.clip(image, a_min=0, a_max=255).astype(dtype='uint8')
        return image