...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # rgb_frame_list, aux_frame_list, anno_frames
        template_rgb_images, template_aux_images, template_anno = dataset.get_frames(seq_id, template_frame_ids, seq_info_dict)
        search_rgb_images, search_aux_images,  search_anno = dataset.get_frames(seq_id, search_frame_ids, seq_info_dict)

        data = TensorDict({'template_rgb_images': template_rgb_images,
                'template_aux_images': template_aux_images,
                'template_rgb_anno': template_anno['rgb_bbox'],
                'template_aux_anno': template_anno['aux_bbox'],
                'search_rgb_images': search_rgb_images,
                'search_aux_images': search_aux_images,
                'search_rgb_anno': search_anno['rgb_bbox'],
                'search_aux_anno': search_anno['aux_bbox']})
        # if any None in data values, return all None data, None data will be filtered out in the dataloader collate function
        for v in data.values():     
            if any(x is None for x in v):
                return TensorDict({'template_rgb_images': None,
                    'template_aux_images': None,
                    'template_rgb_anno': None,
                    'template_aux_anno': None,
                    'search_rgb_images': None,
                    'search_aux_images': None,
                    'search_rgb_anno': None,
                    'search_aux_anno': None})
        return self.processing(data)



class MultimodalSampler(TrackingSampler):
    """ See TrackingSampler."""
